...
def get_tfidf(BOW:pd.DataFrame, vocab_df:pd.DataFrame, tf_method:str='max', idf_method:str='standard'):
    """
    Get a TFIDF-weighted document-term matrix from a bag-of-words table.
    Arguments:
        BOW (pd.DataFrame): A DataFrame produced by ``create_bow()``
        vocab_df (pd.DataFrame): A DataFrame with corpus vocab
    ...
    Returns:
        DTCM (pd.DataFrame): A DataFrame with number of times vocab words (terms) appear in a document (e.g., by book and chapter)
        TFIDF (pd.DataFrame): A DataFrame with an unnormalized, zero-filled document-term matrix of TFIDF weights.
        BOW_df (pd.DataFrame): A DataFrame with number of times each term appears in a document, TF (term frequency), TFIDF (term frequency - inverse doc frequency)
        DFIDF (pd.DataFrame): A Series with a vocabulary as index and DFIDF as value.
        vocab (pd.DataFrame): A VOCAB DataFrame with mean TFIDF and max TFIDF based on TFIDF method used, DF (doc frequency), IDF, DFIDF
    """
    ...
    # add measurements to VOCAB
    vocab_df['tfidf_mean' + "_" + BOW.index.names[-2].split("_")[0] + "_" + tf_method] = TFIDF[TFIDF > 0].mean().fillna(0)
    vocab_df['tfidf_max' + "_" + BOW.index.names[-2].split("_")[0] + "_" + tf_method] = TFIDF.max()
    vocab_df['df'] = DF
    vocab_df['idf'] = IDF
    vocab_df['dfidf'] = vocab_df.df * vocab_df.idf

    return DTCM, TFIDF, BOW, DFIDF, vocab_df
