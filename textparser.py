...
class TextParser():
    ...
    def __init__(self, src_file:str, ohco_pats:[], clip_pats:[], use_nltk=True,dups=False):
        ...
        if self.src_imported:
            ...
            for i, level in enumerate(self.OHCO):
                ...
                if parse_type == 'm':
                    ...
                    # added to remove duplicate chapter headings (exactly the same in table of contents)
                    if self.dups == True:
                        chap_duplicates = self.TOKENS.loc[self.TOKENS.duplicated(keep = 'last') & self.TOKENS.line_str.str.contains(div_pat, case = False)].index.values
